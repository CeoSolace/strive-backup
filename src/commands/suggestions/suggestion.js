...
module.exports = {
  ...
  async messageRun(message, args, data) {
    ensureSuggestionSettings(data.settings);
    ...
    if (sub === "status") {
      ...
      const input = args[1];
      if (!input) return message.safeReply("Please provide a channel or `off`");
      ...
      if (input.toLowerCase() === "off") {
        response = await setApprovedChannel(data.settings, null);
      } else {
        const matched = message.guild.findMatchingChannels(input);
        if (matched.length === 0) response = `No matching channels found for ${input}`;
        else if (matched.length > 1) response = `Multiple channels found for ${input}. Please be more specific.`;
      ...
      if (input.toLowerCase() === "off") {
        response = await setRejectedChannel(data.settings, null);
      } else {
        const matched = message.guild.findMatchingChannels(input);
        if (matched.length === 0) response = `No matching channels found for ${input}`;
        else if (matched.length > 1) response = `Multiple channels found for ${input}. Please be more specific.`;
      ...
      if (!input || !messageId) {
        return message.safeReply("Usage: `suggestion approve <channel> <messageId> [reason]`");
      }

      const matched = message.guild.findMatchingChannels(input);
      if (matched.length === 0) response = `No matching channels found for ${input}`;
      else if (matched.length > 1) response = `Multiple channels found for ${input}. Please be more specific.`;
      else response = await approveSuggestion(message.member, matched[0], messageId, reason);
    }
      ...
      if (!input || !messageId) {
        return message.safeReply("Usage: `suggestion reject <channel> <messageId> [reason]`");
      }

      const matched = message.guild.findMatchingChannels(input);
      if (matched.length === 0) response = `No matching channels found for ${input}`;
      else if (matched.length > 1) response = `Multiple channels found for ${input}. Please be more specific.`;
      else response = await rejectSuggestion(message.member, matched[0], messageId, reason);
    }

    else if (sub === "staffadd") {
      const input = args[1];
      if (!input) return message.safeReply("Please provide a role");
  ...
  async interactionRun(interaction, data) {
    ...
    if (sub === "status") {
      ...
      response = await setChannel(data.settings, channel || null);
    }

    else if (sub === "appch") {
      const channel = interaction.options.getChannel("channel_name");
      response = await setApprovedChannel(data.settings, channel || null);
    }

    else if (sub === "rejch") {
      const channel = interaction.options.getChannel("channel_name");
      response = await setRejectedChannel(data.settings, channel || null);
    }
...
function ensureSuggestionSettings(settings) {
  if (!settings.suggestions) settings.suggestions = {};
  if (!Array.isArray(settings.suggestions.staff_roles)) {
    settings.suggestions.staff_roles = [];
  }
}

async function setStatus(settings, status) {
  ensureSuggestionSettings(settings);
...
async function setChannel(settings, channel) {
  ...
  if (!channel) {
    ...
    await settings.save();
    return "Suggestion channel is now disabled";
  }
