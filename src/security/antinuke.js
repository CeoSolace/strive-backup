// antinuke.js (discord.js v14) - Bright + Top Tier Anti-Spam
//
// Features:
// 1. Bright Review (bots with dangerous perms): kick-first + Accept/Deny
// 2. Anti-nuke counters + lockdown
// 3. Human mass-role stripping defense: derole executor + Restore/Keep panel
// 4. Threat detection: delete + log in #bright-threats + 3-button control panel
// 5. NEW: Top-Tier Anti-Spam (Flood, Duplicate, Mention, Link spam) with Escalation
//
// NO MONGODB:
// Restore data and threat evidence stored in-server as base64 JSON "capsules".
...
module.exports = (client) => {
  ...
  const ROLE_STRIP_WINDOW = 180_000; 
  const ROLE_STRIP_THRESHOLD = 5;

  // =========================
  // SPAM CONFIGURATION (NEW)
  // =========================
  const SPAM_CONFIG = {
    // Flood: Max messages allowed within this window (ms)
    floodWindow: 3000, 
    floodLimit: 5, 
    
    // Duplicate: Max identical messages allowed within this window (ms)
    duplicateWindow: 5000,
    duplicateLimit: 3,
    
    // Mentions: Max mentions allowed in a single message
    mentionLimit: 5,
    
    // Invite/Link Spam: Max invites/links in this window (ms)
    linkWindow: 10000,
    linkLimit: 3,

    // Escalation Thresholds (Strikes before action)
    warnThreshold: 1,
    timeoutThreshold: 2,
    kickThreshold: 3,
    banThreshold: 4,
    ...
    defaultTimeout: 10 * 60 * 1000, // 10 mins
  };
  ...
  // NEW: Spam State Collections
  const spamFloodCache = new Collection(); // key: guild:user -> [{ts}, {ts}]
  const spamDuplicateCache = new Collection(); // key: guild:user -> { lastContent, count, lastTs }
  const spamLinkCache = new Collection(); // key: guild:user -> [{ts}, {ts}]
  const spamStrikeCache = new Collection(); // key: guild:user -> { strikes, lastStrikeAt }
  ...
  setInterval(() => {
    ...
    // NEW: Spam Cleanups
    for (const [key, timestamps] of spamFloodCache.entries()) {
      const filtered = timestamps.filter((t) => now - t < SPAM_CONFIG.floodWindow);
      if (filtered.length === 0) spamFloodCache.delete(key);
      else spamFloodCache.set(key, filtered);
    }
    for (const [key, data] of spamDuplicateCache.entries()) {
      if (now - data.lastTs > SPAM_CONFIG.duplicateWindow) spamDuplicateCache.delete(key);
    }
    for (const [key, timestamps] of spamLinkCache.entries()) {
      const filtered = timestamps.filter((t) => now - t < SPAM_CONFIG.linkWindow);
      if (filtered.length === 0) spamLinkCache.delete(key);
      else spamLinkCache.set(key, filtered);
    }
    // Note: We keep strike cache longer to allow escalation over time, 
    // but could add a decay here if desired (e.g., reset strikes after 1 hour).
  }, 30_000);
  ...
  async function ensureBrightReviewChannel(guild) {
    ...
    if (
      existing &&
      existing.permissionsFor(guild.members.me)?.has([
  ...
  async function postHumanReviewPanel({ guild, executorUser, reason, removedRoles }) {
    ...
    const ownerPing = `<@${ownerId}>`;
    const removedPreview = removedRoles?.length
  ...
  async function handleSpamDetection(message) {
    ...
    if (isWhitelisted(message.guild, message.author, "all")) return; // Whitelisted users ignore spam checks

    const guild = message.guild;
    const user = message.author;
    const key = keyOf(guild.id, user.id);
    const now = Date.now();
    let violationType = null;
    let violationDetails = "";

    // 1. Flood Detection
    if (!spamFloodCache.has(key)) spamFloodCache.set(key, []);
    const floodHistory = spamFloodCache.get(key);
    floodHistory.push(now);
    // Keep only recent timestamps
    const recentFlood = floodHistory.filter((t) => now - t < SPAM_CONFIG.floodWindow);
    spamFloodCache.set(key, recentFlood);
    
    if (recentFlood.length > SPAM_CONFIG.floodLimit) {
      violationType = "Flood";
      violationDetails = `${recentFlood.length} msgs in ${SPAM_CONFIG.floodWindow}ms`;
    }
    ...
    if (!violationType) {
      ...
      if (currentContent.length > 0) {
        if (!spamDuplicateCache.has(key)) {
          spamDuplicateCache.set(key, { lastContent: currentContent, count: 1, lastTs: now });
        } else {
          const data = spamDuplicateCache.get(key);
          if (data.lastContent === currentContent && now - data.lastTs < SPAM_CONFIG.duplicateWindow) {
            data.count++;
            data.lastTs = now;
            if (data.count >= SPAM_CONFIG.duplicateLimit) {
              violationType = "Duplicate";
              violationDetails = `${data.count} identical messages`;
            }
          } else {
            // Reset if content changes or window expires
            spamDuplicateCache.set(key, { lastContent: currentContent, count: 1, lastTs: now });
          }
        }
      }
    ...
    if (!violationType) {
      ...
      if (mentionCount > SPAM_CONFIG.mentionLimit) {
        violationType = "Mention Spam";
        violationDetails = `${mentionCount} mentions`;
      }
    }

    // 4. Link/Invite Spam
    if (!violationType) {
      const inviteRegex = /(https?:\/\/)?(www\.)?(discord\.gg|discordapp\.com\/invite|discord\.com\/invite)\/[a-zA-Z0-9]+/gi;
      ...
      if (hasInvite || hasLink) {
        if (!spamLinkCache.has(key)) spamLinkCache.set(key, []);
        const linkHistory = spamLinkCache.get(key);
        linkHistory.push(now);
        const recentLinks = linkHistory.filter((t) => now - t < SPAM_CONFIG.linkWindow);
        spamLinkCache.set(key, recentLinks);

        if (recentLinks.length > SPAM_CONFIG.linkLimit) {
          violationType = "Link Spam";
          violationDetails = `${recentLinks.length} links/invites in ${SPAM_CONFIG.linkWindow}ms`;
        }
      }
    }
    ...
    if (violationType) {
      ...
      // Handle Escalation
      if (!spamStrikeCache.has(key)) {
        spamStrikeCache.set(key, { strikes: 0, lastStrikeAt: 0 });
      }
      const strikeData = spamStrikeCache.get(key);
      
      // Reset strikes if enough time has passed since last strike (optional, currently persistent until restart/cleanup)
      // For robustness, let's reset if it's been > 1 hour since last strike
      if (now - strikeData.lastStrikeAt > 3600000) {
        strikeData.strikes = 0;
      }

      strikeData.strikes++;
      strikeData.lastStrikeAt = now;
      const currentStrikes = strikeData.strikes;
      ...
      const embed = new EmbedBuilder()
        ...
        .addFields(
          ...
          { name: "Details", value: violationDetails, inline: true },
          { name: "Strike Count", value: `${currentStrikes}`, inline: true }
        )
        .setFooter({ text: `Message Content: ${message.content.slice(0, 50) || "(No text)"}` })
        .setTimestamp();
      ...
      try {
        if (currentStrikes >= SPAM_CONFIG.banThreshold) {
          await member.ban({ reason: `[BRIGHT][SPAM] Ban threshold reached (${violationType})` });
          spamStrikeCache.delete(key); // Reset after ban
        } else if (currentStrikes >= SPAM_CONFIG.kickThreshold) {
          await member.kick({ reason: `[BRIGHT][SPAM] Kick threshold reached (${violationType})` });
          spamStrikeCache.delete(key); // Reset after kick
        } else if (currentStrikes >= SPAM_CONFIG.timeoutThreshold) {
          await member.timeout(SPAM_CONFIG.defaultTimeout, `[BRIGHT][SPAM] Timeout threshold reached (${violationType})`);
        } else if (currentStrikes >= SPAM_CONFIG.warnThreshold) {
          // Send DM Warning
          await user.send(`⚠️ **Warning**: You have been flagged for **${violationType}** in **${guild.name}**. Further violations will result in a timeout, kick, or ban.`).catch(() => {});
        }
      } catch (err) {
        console.error(`[BRIGHT][SPAM] Failed to punish ${user.id}:`, err);
      }
    }
  }
  ...
  client.on("interactionCreate", async (interaction) => {
    ...
    if (prefix === "bth" || prefix === "bth2") {
      ...
      if (prefix === "bth2") {
        ...
        if (action === "timeout24h") return void (await doTimeout(24 * 60 * 1000, "Timeout 24h"));
        if (action === "kick") {
          if (!member.kickable) {
  ...
  client.on("messageCreate", async (message) => {
    ...
    // Run Spam Detection FIRST on all non-bot, non-command messages
    ...
    if (cmd === "=help") {
      await message.reply(
        `**Anti-nuke & Anti-Spam commands**\n` +
          ...
          `• \`all\`\n` +
          `**Features**:\n` +
          `• Auto-kick dangerous bots\n` +
          `• Mass role strip defense\n` +
          `• Channel/Role nuke protection\n` +
          `• Threat word detection\n` +
          `• **Advanced Anti-Spam** (Flood, Dupes, Mentions, Links)`
      );
      return;
    }
